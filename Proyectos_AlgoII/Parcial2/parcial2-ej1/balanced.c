...
static char *take_delimeters(char *expression, size_t *count) {
    ...
    only_delimeters = stack_empty();

    while (expression[i] != '\0') {
        if(is_left_delimiter(expression[i])){
            only_delimeters = stack_push(only_delimeters, expression[i]);
        }
        else if(is_right_delimiter(expression[i])) {
            only_delimeters = stack_push(only_delimeters, expression[i]); 
        }
        i++;
    }
